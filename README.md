...
### 同机部署 Hysteria2 和 Trojan

- 两个 SimpleUI 管理的节点共用同一证书。部署第二个协议时，页面自动选择“复用本机共享证书”，后端和远端 Hook 都会执行证书复用。连接地址可以不同，分享链接会写入共享证书的 SNI。
- 自动证书由 acme.sh 统一申请和续期，两个服务读取 `/etc/simpleui/certificates/current/` 下的同一对文件。HTTP 验证使用 nginx，不会为申请证书停止 Trojan 或 nginx；已有网站配置会保留。如果 nginx 没有标准的 `conf.d` 包含目录，请配置相应 HTTP `server_name`，或选用 DNS / 手动证书。
- 已有双协议服务器需要先升级 Hook，再重新部署其中一个节点，才会迁移旧证书配置。迁移保留另一节点的密码和传输配置。仅更新面板不会修改远端配置；迁移后请更新客户端分享链接中的 SNI。
- 删除单个节点保留共享证书、nginx 和续期配置。手动证书继续引用用户提供的路径，续期仍由用户的证书工具负责，HY2 服务用户需要读取权限。共享证书建立后，日常重新部署请选择原证书模式或复用共享证书；修改证书身份需另行迁移。
- 同一台服务器的部署、更新和卸载互斥。日志清理保留正在执行的任务；面板重启后，未完成任务标为结果未知，需要刷新服务器状态确认。
