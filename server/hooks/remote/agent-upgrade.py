...
def emit(payload):
    print("__SIMPLEUI_RESULT__" + json.dumps(payload, ensure_ascii=False), flush=True)
...
def read_bundle_b64():
    ...
    if bundle_file:
        try:
            ...
            raise SystemExit(0)
    return os.environ.get("SIMPLEUI_UPGRADE_BUNDLE_B64", "")
...
def ensure_tls():
    if not os.path.exists(TLS_CERT) or not os.path.exists(TLS_KEY):
        if not shutil.which("openssl"):
            ...
            emit({"ok": False, "error": "openssl is required"})
            raise SystemExit(0)
        subprocess.run([
            "openssl",
            "req",
    ...
    return fingerprint_proc.stdout.strip().split("=", 1)[-1].replace(":", "")


def update_env(fingerprint):
    env_lines = []
    try:
