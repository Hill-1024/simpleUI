...
const pageDefinitions = [
  { id: "overview", title: "控制台概览", description: "查看服务器、Hook、节点、连接与最近任务的整体状态。" },
...
function groupedRows(items = [], grouped, allLabel, itemKey) {
  if (!grouped) {
  ...
  for (const item of items || []) {
    const key = groupKey(item.group);
    if (!groups.has(key)) groups.set(key, { key, label: groupLabel(item.group), items: [] });
...
<template>
  <div class="app">
    <aside class="sidebar">
      ...
      <nav>
        <a href="#servers"><Server :size="17" />服务器</a>
        <a href="#deploy"><CloudLightning :size="17" />部署</a>
        <a href="#tools"><Gauge :size="17" />工具</a>
        <a href="#nodes"><Wifi :size="17" />节点</a>
        <a href="#connections"><Ban :size="17" />连接封禁</a>
        <a href="#hooks"><Terminal :size="17" />日志</a>
      </nav>
    </aside>
    ...
    <main>
      <header class="topbar">
        <div>
          <h1>服务器 hooks 与节点控制台</h1>
          <p>先通过 SSH 为服务器安装持久化 hook agent；安装完成后，部署、状态刷新、重启和来源 IP 封禁都通过远端 hook 执行。</p>
        </div>
      ...
      <section class="summary-bar">
        <span>服务器 {{ state.servers.length }}</span>
        <span>Hook 就绪 {{ readyServers.length }}</span>
        <span>节点 {{ state.nodes.length }}</span>
        <span>连接来源 {{ state.connections.length }}</span>
        <span>远程 IP {{ state.remoteTraffic.length }}</span>
        <span>流量 {{ fmtBytes(totalTraffic.tx + totalTraffic.rx) }}</span>
      </section>
      ...
      <section id="servers" class="panel">
        ...
        <div v-else class="table-wrap server-table">
          <table>
            <thead>
              <tr><th>服务器</th><th>主机</th><th>资源</th><th>网络</th><th>状态</th><th>地区</th><th>同步</th><th>操作</th></tr>
            </thead>
            <tbody>
              <tr v-for="server in state.servers" :key="server.id">
                <td v-if="editingServerId === server.id">
                  <input v-model="serverEditForm.name" class="inline-input" required />
                  <small>{{ server.sshUserHint || "root" }}@{{ server.host }}</small>
                </td>
                <td v-else><strong>{{ server.name }}</strong><small>{{ server.sshUserHint || "root" }}@{{ server.host }}</small></td>
                <td v-if="editingServerId === server.id" class="inline-fields">
                ...
                <td v-else>{{ server.host }}:{{ server.port || 22 }}</td>
                <td class="metric-cell">
                  <strong>CPU {{ fmtPercent(server.metrics?.cpu?.usagePercent) }}</strong>
                  <small>Mem {{ fmtPercent(server.metrics?.memory?.usedPercent) }} · Disk {{ fmtPercent(server.metrics?.disk?.usedPercent) }}</small>
                  <small>Load {{ server.metrics?.cpu?.load1 ?? "-" }} / {{ server.metrics?.cpu?.cores || "-" }} 核</small>
                </td>
                <td class="metric-cell">
                  <strong>↓ {{ fmtRate(server.metrics?.network?.rxRate || 0) }}</strong>
                  <small>↑ {{ fmtRate(server.metrics?.network?.txRate || 0) }}</small>
                  <small>{{ fmtBytes(server.metrics?.network?.rx || 0) }} / {{ fmtBytes(server.metrics?.network?.tx || 0) }}</small>
                </td>
                <td><span :class="`pill status-${server.hookStatus || server.status || 'unknown'}`">{{ statusLabel(server.hookStatus || server.status) }}</span></td>
                <td v-if="editingServerId === server.id"><input v-model="serverEditForm.location" class="inline-input short-input" /></td>
                <td v-else>{{ server.location || "-" }}</td>
                <td v-if="editingServerId === server.id">
                  <input v-model.number="serverEditForm.hookPort" class="inline-input short-input" type="number" />
                  <small>{{ server.hookUrl || "未安装" }}</small>
                </td>
                <td v-else class="metric-cell">
                  <small>{{ fmtTime(server.metrics?.updatedAt) }}</small>
                  <small v-if="server.metrics?.lastSyncError" class="error-text">{{ server.metrics.lastSyncError }}</small>
                  <small v-else>{{ server.hookUrl || "未安装" }}</small>
                </td>
                <td>
                  <div class="row-actions">
                    <template v-if="editingServerId === server.id">
                      <button type="button" title="保存服务器信息" :disabled="busy" @click="saveServer(server)"><Save :size="15" /></button>
                      <button type="button" title="取消编辑" :disabled="busy" @click="cancelEditServer"><X :size="15" /></button>
                    </template>
                    <template v-else>
                      <button type="button" title="升级/重装 hook" :disabled="busy" @click="prepareHookUpgrade(server)"><ShieldCheck :size="15" /></button>
                      <button type="button" title="编辑服务器信息" :disabled="busy" @click="startEditServer(server)"><Pencil :size="15" /></button>
                      <button class="icon-danger" type="button" title="卸载 hook 并删除服务器" :disabled="busy || server.hookStatus === 'deleting'" @click="deleteServer(server)">
                        <Trash2 :size="15" />
                      </button>
                      <button class="icon-danger" type="button" title="强制清除本地服务器记录" :disabled="busy" @click="forceClearServer(server)">
                        <Eraser :size="15" />
                      </button>
                    </template>
                  </div>
      ...
      <section id="nodes" class="panel">
        ...
        <div v-else class="table-wrap">
          <table>
            <thead>
              <tr>
                <th></th><th>节点</th><th>服务器</th><th>入口</th><th>流量</th><th>在线</th><th>状态</th><th>操作</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="node in state.nodes" :key="node.id">
                <td><input type="checkbox" :checked="selectedNodes.includes(node.id)" @change="toggleNode(node.id, $event.target.checked)" /></td>
                <td><strong>{{ node.name }}</strong><small>{{ node.protocol }}</small></td>
                <td>{{ serverName(node.serverId) }}</td>
                <td>{{ node.endpoint }}</td>
                <td>{{ fmtBytes((node.traffic?.tx || 0) + (node.traffic?.rx || 0)) }}</td>
                <td>{{ node.onlineUsers || 0 }}</td>
                <td class="metric-cell">
                  <span :class="`pill status-${node.status || 'unknown'}`">{{ statusLabel(node.status) }}</span>
                  <small>{{ fmtTime(node.lastCheckedAt) }}</small>
                  <small v-if="node.lastSyncError" class="error-text">{{ node.lastSyncError }}</small>
                </td>
                <td>
                  <div class="row-actions">
                    <button type="button" title="修改节点参数" :disabled="busy || !readyServerIds.has(node.serverId)" @click="startEditNode(node)"><Pencil :size="15" /></button>
                    <button type="button" title="刷新状态" :disabled="!readyServerIds.has(node.serverId)" @click="refreshNode(node)"><RefreshCcw :size="15" /></button>
                    <button type="button" title="重启服务" :disabled="!readyServerIds.has(node.serverId)" @click="serviceNode(node, 'restart')"><RotateCw :size="15" /></button>
                    <button class="icon-danger" type="button" title="卸载并删除节点" :disabled="busy || !readyServerIds.has(node.serverId) || node.status === 'deleting'" @click="deleteNode(node)"><Trash2 :size="15" /></button>
                    <button class="icon-danger" type="button" title="强制清除本地节点记录" :disabled="busy" @click="forceClearNode(node)"><Eraser :size="15" /></button>
                  </div>
                </td>
              </tr>
            </tbody>
      ...
      <section id="connections" class="panel">
        ...
        <div class="ban-panel">
          ...
          <small>该动作通过各服务器的持久化 hook 写入防火墙 DROP 规则，不是 WebUI 访问控制。</small>
        </div>
        ...
        <template v-else>
          ...
          <div v-else class="table-wrap compact-table stats-table-wrap">
            <table>
              ...
              <tbody>
                <tr v-for="item in filteredRemoteTraffic" :key="item.id">
                  <td><strong>{{ item.remoteIp }}</strong><small v-if="item.ipFamily">IPv{{ item.ipFamily }}</small></td>
                  <td>{{ nodeName(item.nodeId) }}</td>
                  <td>{{ fmtBytes(item.rx) }}</td>
                  <td>{{ fmtBytes(item.tx) }}</td>
                  <td>{{ fmtBytes(item.total || ((item.rx || 0) + (item.tx || 0))) }}</td>
                  <td>{{ item.connections || 0 }}</td>
                  <td>{{ (item.protocols || []).join(", ") || "-" }}</td>
                  <td>{{ fmtTime(item.lastSeenAt) }}</td>
                  <td><button type="button" @click="sourceIp = item.remoteIp">填入封禁</button></td>
                </tr>
              </tbody>
            </table>
          </div>
            ...
            <table>
              ...
              <tbody>
                <tr v-for="connection in filteredConnections" :key="connection.id">
                  <td><strong>{{ connection.sourceIp }}</strong><small v-if="connection.ipFamily">IPv{{ connection.ipFamily }}</small></td>
                  <td>{{ nodeName(connection.nodeId) }}</td>
                  <td>{{ connection.remote }}</td>
                  <td>{{ connection.local }}</td>
                  <td>{{ connection.protocol }}</td>
                  <td><button type="button" @click="sourceIp = connection.sourceIp">填入</button></td>
                </tr>
              </tbody>
            </table>
          </div>
