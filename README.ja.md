...
- 永続 Hook は自己署名 HTTPS と証明書フィンガープリント固定を使います。Hook ポートは引き続きファイアウォール、VPN、または SSH tunnel 経由だけで公開してください。
...
本プロジェクトのソースコードは GNU Affero General Public License v3.0（AGPL-3.0）に基づいて公開されています。詳細は [LICENSE](./LICENSE) を参照してください。再利用、変更、再配布、またはネットワークサービスとして提供する場合は AGPL-3.0 に従ってください。第三者サービス、リモートノード設定、ユーザーデータは、このリポジトリのライセンスによって自動的に許諾されるものではありません。
